...
return ("<h1 class='pagecontenttitle{$direct_settings['theme_css_corners']}'>".(direct_local_get ("core_done")).": {$direct_cachedata['output_command']}</h1>
<div class='pagecontent'>{$direct_cachedata['output_return_value']}</div>");
}
...
function direct_output_oset_cp_daemon_entry ()
{
	...
	$f_title = $direct_cachedata['output_entry']['name'];
	if (isset ($direct_cachedata['output_entry']['identifier'])) { $f_title .= " - <span style='font-size:10px'>{$direct_cachedata['output_entry']['identifier']}</span>"; }
...
function direct_output_oset_cp_daemon_status ()
{
	...
	$f_return = "<h1 class='pagecontenttitle{$direct_settings['theme_css_corners']}'>".(direct_local_get ("cp_daemon_status"))."</h1>\n<p class='pageborder{$direct_settings['theme_css_corners']} pagebg pagecontent' style='padding:$direct_settings[theme_form_td_padding];text-align:center'><b>".(direct_local_get ("cp_daemon_uptime")).":</b> ".($direct_globals['basic_functions']->datetime ("shortdate&time",$direct_cachedata['output_uptime'],$direct_settings['user']['timezone'],(direct_local_get ("datetime_dtconnect"))));
